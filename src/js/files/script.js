...
if (searchButton) {
    ...
    window.addEventListener('click', e => {
        ...
        if (!target.closest('.search-button-mob') && !target.closest('.search-top-header__simplebar') && !target.closest('.form__icon') && !target.closest('.form__input input')) {
            document.documentElement.classList.remove("_search-open");
        }
    })
}
        ...
        function showBlogItems(filter) {
            blogItems.forEach(blogItem => {
                if (filter === 'all' || !filter) {
                    ...
                    if (blogItem.classList.contains('poster__column_' + filter)) {
                        blogItem.classList.remove('_hide');
                    }
                }
            });
...
if (selectTicketsItem) {
    ...
    const tableTop = document.querySelector('.time-table__top h5');
